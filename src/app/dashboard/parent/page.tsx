...
const mockSubjectsProgress: SubjectProgress[] = [
  {
    ...
    topicsCovered: 8,
    topicsTotal: 12,
    lastStudiedFile: "Chapter 3 Notes.pdf",
    quizAttempts: 3,
    avgScore: 75,
  },
  {
    ...
    topicsCovered: 5,
    topicsTotal: 10,
    lastStudiedFile: "Newton's Laws.ppt",
    quizAttempts: 1,
    avgScore: 90,
  },
  {
    ...
    topicsCovered: 10,
    topicsTotal: 10,
    lastStudiedFile: "Periodic Table.docx",
    quizAttempts: 5,
    avgScore: 82,
  },
  {
    ...
    topicsCovered: 2,
    topicsTotal: 15,
    lastStudiedFile: "Cell Structure.pdf",
    quizAttempts: 0,
    avgScore: 0,
  },
];
...
function ParentDashboardPage() {
  ...
  // On mount, parse query params for names
  useEffect(() => {
  ...
  // Simulate logout with toast and redirect
  const handleLogout = () => {
    toast({
      title: "Logged Out",
  ...
  // Toast for asking AI questions on a specific file
  ...
  // Toast for viewing detailed subject report
            ...
            <nav className="space-y-1.5">
              ...
              <Button
                ...
                onClick={() =>
                  toast({
                    title: "Coming Soon",
                    description: "Student profile & settings will be here.",
                  })
                }
              >
            ...
            <CardContent className="space-y-4 sm:space-y-6">
              {mockSubjectsProgress.length === 0 ? (
                <p className="text-muted-foreground text-center py-4">
                  No subject data available for {studentName} yet.
                </p>
              ) : (
                    ...
                    >
                        ...
                        <p className="text-muted-foreground">
                          ...
                          <span
                            className={`font-medium ${
                              subject.avgScore >= 70 ? "text-green-600" : "text-orange-600"
                            }`}
                          >
                            {subject.avgScore > 0 ? `${subject.avgScore}%` : "N/A"}
                          </span>
                        </p>
                      </CardContent>
                      <CardFooter className="flex flex-col sm:flex-row gap-2 pt-3 sm:pt-4">
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleAskQuestionOnFile(subject.name, subject.lastStudiedFile)
                          }
                          className="w-full sm:w-auto"
                        >
                          <MessageCircleQuestion className="mr-1.5 h-4 w-4" /> Ask AI about "
                          {truncateFileName(subject.lastStudiedFile)}"
                        </Button>
                        <Button
                          variant="ghost"
                          size="sm"
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm sm:text-base">
              <div className="p-4 bg-muted/50 rounded-lg shadow-sm">
                ...
                <p className="text-muted-foreground text-xs sm:text-sm">
                  Create quizzes on specific weak areas identified from reports.
                </p>
              </div>
              <div className="p-4 bg-muted/50 rounded-lg shadow-sm">
              ...
              <div className="p-4 bg-muted/50 rounded-lg shadow-sm">
                ...
                <p className="text-muted-foreground text-xs sm:text-sm">
                  Suggest supplementary materials for challenging topics.
                </p>
              </div>
              <div className="p-4 bg-muted/50 rounded-lg shadow-sm">
                <h4 className="font-semibold mb-1">Goal Setting & Tracking</h4>
                <p className="text-muted-foreground text-xs sm:text-sm">
                  Collaborate with {studentName} to set and monitor academic goals.
                </p>
              </div>
            </CardContent>
          </Card>
 ...
 * E.g. "VeryLongFileNameThatExceedsLimit.pdf" -> "VeryLongFileName...pdf"
 */
function truncateFileName(name: string, maxLength: number = 20): string {
  ...
  // If baseName fits with ellipsis + extension, truncate; else return full name
