...
const generateTopicImageFlow = ai.defineFlow(
  ...
  async (input: GenerateTopicImageInput) => {
    const {media} = await ai.generate({
      model: 'googleai/gemini-2.0-flash-preview-image-generation', // Updated model for image generation
      ...
      config: {
        responseModalities: ['TEXT', 'IMAGE'], // Must include IMAGE for this model
      },
    });
