...
export function KeyPointGenerator() {
  ...
  const handlePdfFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setPdfLoading(true);
    toast({ title: "Processing PDF", description: "AI is extracting text from your PDF..." });

    const result = await handlePdfUploadForKeyPoints(file);

    if (result.error) {
      toast({ title: "PDF Error", description: result.error, variant: "destructive" });
    } else if (result.extractedText) {
      setAnswerContent(result.extractedText);
      toast({ title: "Text Extracted", description: "Text from the PDF has been loaded into the text area." });
    } else {
      toast({ title: "No Text Found", description: "Could not extract any text from the PDF.", variant: "destructive" });
    }
    setPdfLoading(false);
    ...
    <Card className="shadow-lg w-full max-w-3xl mx-auto">
        ...
        <div>
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-2 gap-2">
            ...
            <Button
              ...
              disabled={pdfLoading}
            >
              {pdfLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <FileUp className="mr-2 h-4 w-4" />}
          ...
          <Textarea
            ...
            placeholder="Paste your answer content here, or upload a PDF..."
            rows={12}
      ...
      <CardFooter>
        <Button 
          ...
          disabled={isLoading || pdfLoading || wordCount === 0 || wordCount > MAX_WORDS_KEY_POINTS || !!error} 
